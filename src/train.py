...
device = 1
# if os.path.exists("best_net.pt"):
#     # 报错：没有cuda，加上：map_location='cpu'
#     model.load_state_dict(torch.load("best_net.pt", map_location='cpu'))
# use_cuda = torch.cuda.is_available()
# device = torch.device("cuda" if use_cuda else "cpu")
# model = Net().to(device)
# if os.path.exists("car_plate.pt"):
#     # 报错：没有cuda，加上：map_location='cpu'
#     model.load_state_dict(torch.load("car_plate.pt", map_location='cpu'))
...
for epoch in range(0, train_number_epochs):
    for i, data in enumerate(train_dataloader, 0):
        ...
        img0, img1 , label = img0, img1, label #数据移至GPU
        optimizer.zero_grad()
